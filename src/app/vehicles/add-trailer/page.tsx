...
import { ChevronLeft, Truck as TrailerIcon, User, CalendarDays, Clock } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';

interface TrailerInfo {
  truckNumber: string;
  lastDriverTrailer: string;
  lastUsedDateTrailer: string;
  lastDropOffTimeTrailer: string;
}
...
export default function AddTrailerPage() {
  ...
  useEffect(() => {
    ...
    setTrailerInfo({
      ...
      lastDriverTrailer: "John Doe",
      lastUsedDateTrailer: "2024-07-14",
      lastDropOffTimeTrailer: "10:00 AM",
    });
    setIsLoading(false);
  }, []);
      ...
      <div className="flex-grow flex flex-col items-center">
        <Card className="w-full max-w-2xl shadow-lg rounded-lg border-none">
          <CardHeader className="items-center text-center">
            <TrailerIcon className="h-12 w-12 text-primary mb-2" />
            <CardTitle className="text-2xl font-heading-style">
              Add Trailer
            </CardTitle>
            <CardDescription>For Truck: {trailerInfo.truckNumber}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-1 pt-4 text-sm sm:text-base">
            <InfoRow label="Last Driver (Trailer)" value={trailerInfo.lastDriverTrailer} icon={User} />
            <InfoRow label="Last Used Date (Trailer)" value={trailerInfo.lastUsedDateTrailer} icon={CalendarDays} />
            <InfoRow label="Last Drop-off Time (Trailer)" value={trailerInfo.lastDropOffTimeTrailer} icon={Clock} />
            
            <div className="pt-6 mt-4 border-t border-border">
              <h3 className="text-lg font-semibold text-center mb-4 text-foreground">Enter New Trailer Information</h3>
              ...
              <Button className="w-full" size="lg" onClick={handleAddTrailer}>
